...
-- Rollback for 20260917214500_order_archived_immutability.sql
--
-- Restores the six editing RPCs to their pre-E1 definitions:
--   change_order_content, change_order_management,
--   change_order_notification_status, assign_order_client,
--   create_client_and_assign_order  -> 20260907171432_remote_schema.sql
--   change_order_details            -> 20260914163515_stores-and-order-store-id.sql
--
-- WARNING: after this rollback, archived orders are editable again through
-- these RPCs. The "archived = read-only" guarantee falls back to the browser.
--
-- Leaves Lifecycle V1 (change_order_status, archive_order,
-- tg_orders_lifecycle_guard, tg_activity_log_order_archived) and every Kiosk
-- object untouched.

begin;

-- ---------------------------------------------------------------------------
-- change_order_content (pre-E1)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- change_order_management (pre-E1)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- change_order_notification_status (pre-E1)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- create_client_and_assign_order (pre-E1)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Drop the E1 contract comments
-- ---------------------------------------------------------------------------

COMMENT ON FUNCTION public.change_order_content(uuid, text, text, uuid) IS NULL;
COMMENT ON FUNCTION public.change_order_details(uuid, text, text, uuid) IS NULL;
COMMENT ON FUNCTION public.change_order_management(uuid, text, uuid, uuid) IS NULL;
COMMENT ON FUNCTION public.change_order_notification_status(uuid, text, uuid) IS NULL;
COMMENT ON FUNCTION public.assign_order_client(uuid, uuid, uuid) IS NULL;
COMMENT ON FUNCTION public.create_client_and_assign_order(uuid, uuid, text, text, text, text, text, text, text, uuid) IS NULL;
