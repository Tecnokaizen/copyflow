-- Order Editing V1 · E2A — optimistic concurrency
--
-- Adds public.orders.row_version and coexisting *_v2 RPCs that require
-- p_expected_version. v1 RPCs, archive_order, lifecycle guard, activity
-- triggers and Kiosk objects are not rewritten.
...
COMMENT ON COLUMN public.orders.row_version IS
  'Monotonic per-row write counter. Opaque concurrency token for *_v2 RPCs.';
...
COMMENT ON FUNCTION public.bump_row_version() IS
  'BEFORE UPDATE: increments orders.row_version on every successful row update.';

DROP TRIGGER IF EXISTS orders_bump_row_version ON public.orders;
CREATE TRIGGER orders_bump_row_version
  BEFORE UPDATE ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.bump_row_version();

REVOKE ALL ON FUNCTION public.bump_row_version() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.bump_row_version() TO postgres;
...
CREATE OR REPLACE FUNCTION public.change_order_details_v2 (
  p_order_id  uuid,
  p_field     text,
  p_value     text,
  p_tenant_id uuid,
...
CREATE OR REPLACE FUNCTION public.change_order_management_v2 (
  p_order_id  uuid,
  p_field     text,
  p_value_id  uuid,
  p_tenant_id uuid,
...
CREATE OR REPLACE FUNCTION public.change_order_notification_status_v2 (
  p_order_id            uuid,
...
CREATE OR REPLACE FUNCTION public.assign_order_client_v2 (
  p_order_id  uuid,
  p_client_id uuid,
  p_tenant_id uuid,
  p_expected_version bigint
...
CREATE OR REPLACE FUNCTION public.create_client_and_assign_order_v2 (
  p_order_id         uuid,
...
CREATE OR REPLACE FUNCTION public.change_order_status_v2 (
  p_order_id  uuid,
  p_status_id uuid,
  p_tenant_id uuid,
  p_expected_version bigint
...
COMMENT ON FUNCTION public.change_order_content_v2(uuid, text, text, uuid, bigint) IS
  'INVOKER. v2 of change_order_content. Requires p_expected_version. Rejects stale rows with GCO01 after the archived guard.';
...
COMMENT ON FUNCTION public.change_order_details_v2(uuid, text, text, uuid, bigint) IS
  'INVOKER. v2 of change_order_details. Requires p_expected_version. Rejects stale rows with GCO01 after the archived guard.';
REVOKE ALL ON FUNCTION public.change_order_details_v2(uuid, text, text, uuid, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.change_order_details_v2(uuid, text, text, uuid, bigint) TO authenticated, postgres;

COMMENT ON FUNCTION public.change_order_management_v2(uuid, text, uuid, uuid, bigint) IS
  'INVOKER. v2 of change_order_management. Requires p_expected_version. Rejects stale rows with GCO01 after the archived guard.';
REVOKE ALL ON FUNCTION public.change_order_management_v2(uuid, text, uuid, uuid, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.change_order_management_v2(uuid, text, uuid, uuid, bigint) TO authenticated, postgres;

COMMENT ON FUNCTION public.change_order_notification_status_v2(uuid, text, uuid, bigint) IS
  'INVOKER. v2 of change_order_notification_status. Requires p_expected_version. Rejects stale rows with GCO01 after the archived guard.';
REVOKE ALL ON FUNCTION public.change_order_notification_status_v2(uuid, text, uuid, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.change_order_notification_status_v2(uuid, text, uuid, bigint) TO authenticated, postgres;

COMMENT ON FUNCTION public.assign_order_client_v2(uuid, uuid, uuid, bigint) IS
  'INVOKER. v2 of assign_order_client. Requires p_expected_version. Rejects stale rows with GCO01 after the archived guard.';
REVOKE ALL ON FUNCTION public.assign_order_client_v2(uuid, uuid, uuid, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.assign_order_client_v2(uuid, uuid, uuid, bigint) TO authenticated, postgres;

COMMENT ON FUNCTION public.create_client_and_assign_order_v2(uuid, uuid, text, text, text, text, text, text, text, uuid, bigint) IS
  'INVOKER. v2 of create_client_and_assign_order. Requires p_expected_version. Locks the order and rejects stale/archived before creating the client.';
REVOKE ALL ON FUNCTION public.create_client_and_assign_order_v2(uuid, uuid, text, text, text, text, text, text, text, uuid, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_client_and_assign_order_v2(uuid, uuid, text, text, text, text, text, text, text, uuid, bigint) TO authenticated, postgres;

COMMENT ON FUNCTION public.change_order_status_v2(uuid, uuid, uuid, bigint) IS
  'INVOKER. v2 of change_order_status. Requires p_expected_version. Rejects stale rows with GCO01 after the archived guard.';
REVOKE ALL ON FUNCTION public.change_order_status_v2(uuid, uuid, uuid, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.change_order_status_v2(uuid, uuid, uuid, bigint) TO authenticated, postgres;
